...
# Import exceptions
from excel_mcp_server.src.excel_mcp.exceptions import (
    ValidationError,
    WorkbookError,
    SheetError,
    DataError,
...
from excel_mcp_server.src.excel_mcp.workbook import create_workbook as create_workbook_impl
# Import from excel_mcp package with consistent _impl suffixes
from excel_mcp_server.src.excel_mcp.validation import (
    validate_formula_in_cell_operation as validate_formula_impl,
    validate_range_in_sheet_operation as validate_range_impl
)
from excel_mcp_server.src.excel_mcp.chart import create_chart_in_sheet as create_chart_impl
from excel_mcp_server.src.excel_mcp.workbook import get_workbook_info
from excel_mcp_server.src.excel_mcp.formatting import format_range as format_range_func
from excel_mcp_server.src.excel_mcp.data import write_data
from excel_mcp_server.src.excel_mcp.workbook import create_sheet as create_worksheet_impl
from excel_mcp_server.src.excel_mcp.data import read_excel_range
from excel_mcp_server.src.excel_mcp.pivot import create_pivot_table as create_pivot_table_impl
from excel_mcp_server.src.excel_mcp.calculations import apply_formula as apply_formula_impl
from excel_mcp_server.src.excel_mcp.sheet import (
    copy_sheet,
    delete_sheet,
    rename_sheet,
    merge_range,
...
def find_record_position(filepath: str, sheet_name: str, identifier: dict) -> tuple:
    """Find row position of a record based on identifier"""
    data = read_data_from_excel(filepath, sheet_name)
    headers = data[0]
    ...
    for i, row in enumerate(data[1:]):
        ...
        for key, value in identifier.items():
            ...
            if str(row[col_index]) != str(value):
                match = False
                break
        if match:
...
def update_data_in_excel(
    filepath: str,
    sheet_name: str,
    identifier: dict,
    update_fields: dict
) -> str:
    """Update existing record in Excel"""
    try:
        row_index = find_record_position(filepath, sheet_name, identifier)
...
def copy_range(
    ...
    try:
        full_path = get_excel_path(filepath)
        from excel_mcp_server.src.excel_mcp.sheet import copy_range_operation
...
def delete_range(
    ...
    try:
        full_path = get_excel_path(filepath)
        from excel_mcp_server.src.excel_mcp.sheet import delete_range_operation
