...
# Import your Nexra tools (if available)
from nexra.src.tools import response_tool, places_tool, socials_tool, instagram_tool, youtube_tool, food_orders_tool
...
async def query_stream_generator(session_id, user_input, bot,top_k):
    ...
    if bot == "agni":
        context = await scrape_content_agni()
        print(context)
    else:
        context = None
    # 1. Manager decision
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("clost_web_server:app", host="0.0.0.0", port=8000, loop="asyncio")